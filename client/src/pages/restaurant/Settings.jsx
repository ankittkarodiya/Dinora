...
export default function Settings() {
  ...
  // ── THE CHANGE: replaces restaurant/subscription/loading/fetchError
  // useState, hasFetchedRef, its useEffect, and fetchAll with two
  // useQuery calls. The restaurant query uses key ["restaurant"] — the
  // SAME key Tables.jsx uses — so editing your profile here is instantly
  // visible there too, with zero extra fetch. fetchWithRetry is preserved
  // inside each queryFn, same retry-on-failure behavior as before, just
  // applied per-query instead of to one combined Promise.all.
  const {
    data: restaurant,
    ...
    queryKey: ["restaurant"],
    queryFn: async () => {
      const res = await fetchWithRetry(() => getMyRestaurantApi());
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetchWithRetry(() => getSubscriptionStatusApi());
      return res.subscription;
    },
  });

  const loading = restaurantLoading || subscriptionLoading;
  const fetchError = restaurantError || subscriptionError;
  ...
  // ── THE CHANGE: since `restaurant` now arrives asynchronously via
  // useQuery (rather than being set directly inside a manual fetchAll),
  // this effect syncs the editable form fields the moment restaurant
  // data becomes available — same initialization the original fetchAll
  // did inline, just triggered off the query's data instead.
  useEffect(() => {
    ...
    if (restaurant.razorpayKeyId) {
      setRazorpayForm({ keyId: restaurant.razorpayKeyId, keySecret: "" });
    }
...
const handleDisablePush = async () => {
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      // ← patches the shared ["restaurant"] cache directly — Tables.jsx
      // and anywhere else reading this same query see the update instantly
      queryClient.setQueryData(["restaurant"], data.restaurant);
  ...
  const handleSaveRazorpay = async (e) => {
    ...
    try {
      ...
      // ← patches the shared ["restaurant"] cache directly
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            // refetch straight into the cache, so this page AND anywhere
            // else reading ["subscription"] gets the fresh status
            const subData = await getSubscriptionStatusApi();
            queryClient.setQueryData(["subscription"], subData.subscription);
          } catch (err) {
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4">
              ...
              <button
                ...
                // new for disable the btn if no changes done
                className="px-6 py-3 bg-blue-600 enabled:hover:bg-blue-700 text-white rounded-xl font-bold text-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
            ...
            <form onSubmit={handleSaveRazorpay} className="space-y-3">
              ...
              <button
                ...
                // new for disable the btn if no changes done
                className="w-full py-2.5 bg-blue-600 enabled:hover:bg-blue-700 text-white rounded-xl font-bold text-xs disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {linkingRazorpay
                  ? "Linking..."
                  : restaurant?.razorpayLinked
