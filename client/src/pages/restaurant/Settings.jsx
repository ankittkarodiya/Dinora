...
export default function Settings() {
  ...
  const [profileForm, setProfileForm] = useState({
    name: "", description: "", phone: "", address: "", gstNumber: "", gstPercent: "5",
  });
  ...
  const fetchAll = async () => {
    try {
      ...
      setProfileForm({
        ...
        gstNumber: rest.gstNumber || "", gstPercent: String(rest.gstPercent || "5"),
      });
      if (rest.razorpayKeyId) setRazorpayForm({ keyId: rest.razorpayKeyId, keySecret: "" });
  ...
  const handleSaveRazorpay = async (e) => {
    ...
    try {
      ...
      setRestaurant((p) => ({ ...p, razorpayLinked: true, razorpayKeyId: razorpayForm.keyId }));
  ...
  const handleSubscribe = async (planId) => {
    if (!window.Razorpay) return toast.error("Payment gateway not loaded. Refresh the page.");
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            await verifySubscriptionApi({ ...response, plan: planId, billingCycle });
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-slate-400 text-sm animate-pulse">Loading settings...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Manage your restaurant, subscription, and payments</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
            <h3 className="text-white font-bold text-base mb-5">Restaurant Profile</h3>
            <form onSubmit={handleSaveProfile} className="space-y-4">
              <div className="flex gap-4 items-start">
                <input id="settings-logo-input" type="file" accept="image/*" onChange={handleLogoChange} className="hidden" />
                <div className="flex-1">
                  <label className="block text-slate-300 text-xs font-bold uppercase tracking-wide mb-1.5">Restaurant Name</label>
                  <input
              ...
              <div>
                <label className="block text-slate-300 text-xs font-bold uppercase tracking-wide mb-1.5">Description</label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-slate-300 text-xs font-bold uppercase tracking-wide mb-1.5">Phone</label>
                  <input
                ...
                <div>
                  <label className="block text-slate-300 text-xs font-bold uppercase tracking-wide mb-1.5">Address</label>
                  <input
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-slate-300 text-xs font-bold uppercase tracking-wide mb-1.5">GST Number</label>
                  <input
                ...
                <div>
                  <label className="block text-slate-300 text-xs font-bold uppercase tracking-wide mb-1.5">GST %</label>
                  <input
                    type="number"
                    value={profileForm.gstPercent}
                    onChange={(e) => setProfileForm((p) => ({ ...p, gstPercent: e.target.value }))}
                    className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-white outline-none focus:border-blue-500 text-sm"
                  />
                </div>
              </div>
          ...
          {/* Subscription Plans */}
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
            <h3 className="text-white font-bold text-base mb-1">Subscription Plans</h3>
            <p className="text-slate-400 text-xs mb-4">Upgrade or switch your plan anytime</p>

            {/* Billing cycle toggle */}
            <div className="flex items-center gap-1 bg-white/5 border border-white/10 rounded-xl p-1 mb-4 max-w-xs">
              <button
                ...
                return (
                  <div
                    ...
                    className={`rounded-2xl border-2 p-4 relative transition-all flex flex-col ${
                      isCurrent ? "border-green-500 bg-green-500/10" : "border-white/10 bg-white/5"
                    }`}
                  >
                    ...
                    <div className="text-white font-bold text-base mt-1">{plan.name}</div>
                    <div className="text-slate-300 text-sm mb-1">
                      ₹{price.toLocaleString()}<span className="text-slate-500 text-xs">/{period}</span>
                    </div>
                    {billingCycle === "yearly" && (
                    ...
                    >
                      {isCurrent ? "Active" : payingPlan === plan.id ? "Opening..." : "Choose Plan"}
                    </button>
                  </div>
                );
              })}
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
            <div className="flex items-center gap-2 mb-3">
              <h3 className="text-white font-bold text-sm">Razorpay Integration</h3>
              {restaurant?.razorpayLinked && (
                <span className="text-[10px] bg-green-500/20 text-green-400 border border-green-500/30 px-2 py-0.5 rounded-full font-bold">Linked</span>
              )}
            </div>
            <p className="text-slate-400 text-xs leading-relaxed mb-4">
              Link your own Razorpay account so customer payments go straight to you, not through us.
            </p>
            <form onSubmit={handleSaveRazorpay} className="space-y-3">
              <input
                value={razorpayForm.keyId}
                onChange={(e) => setRazorpayForm((p) => ({ ...p, keyId: e.target.value }))}
              ...
              <PasswordInput
                ...
                onChange={(e) => setRazorpayForm((p) => ({ ...p, keySecret: e.target.value }))}
              ...
              <div className="flex items-start gap-2 mt-3">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-slate-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                </svg>
                <p className="text-xs text-slate-500 leading-relaxed">
                  Your Razorpay Secret Key is encrypted before it's stored. Not even our team can view it once saved — it's only ever used securely, server-side, to process your payments.
                </p>
              </div>
            </form>
          </div>
