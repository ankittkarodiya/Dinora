    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {/* <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5">
          ...
          <div className="space-y-2">
            {[
              { to: "/restaurant/menu",       icon: "🍽️", label: "Add Menu Item",   sub: `${menuItems.length} items` },
              { to: "/restaurant/categories", icon: "📁", label: "Add Category",    sub: `${categories.length} categories` },
              { to: "/restaurant/tables",     icon: "🪑", label: "Manage Tables",   sub: `${tables.length} tables` },
              { to: "/restaurant/orders",     icon: "📋", label: "View Orders",     sub: `${pendingOrders} pending` },
              { to: "/restaurant/reviews",    icon: "⭐", label: "View Reviews",    sub: isPro ? `${reviews.length} reviews` : "Pro feature" },
            ].map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className="flex items-center gap-3 p-3 rounded-xl border border-white/10 bg-white/5 hover:bg-white/15 transition-all hover:-translate-y-0.5 group"
              >
                <span className="text-xl">{link.icon}</span>
                <div className="flex-1">
                  <div className="text-white font-semibold text-sm group-hover:text-blue-300 transition-colors">{link.label}</div>
...
//   return (
//     <div className={`rounded-2xl border bg-linear-to-br ${colors[color]} backdrop-blur-sm p-5`}>
...
//   const fetchAll = async () => {
//       try {
//         const [o, m, c, t, r] = await Promise.all([
//           getOrdersApi(),
//           getMenuItemsApi(),
//           getCategoriesApi(),
//           getTablesApi(),
...
//       {/* Order status overview */}
//       <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5">
//         <h3 className="text-white font-bold text-base mb-5">Order Status Overview</h3>
//         <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
//           {[
//             { label: "Pending",   count: pendingOrders,                                        color: "text-amber-400",  bg: "bg-amber-400/10",  border: "border-amber-400/20"  },
//             { label: "Preparing", count: preparingOrders,                                      color: "text-blue-400",   bg: "bg-blue-400/10",   border: "border-blue-400/20"   },
//             { label: "Ready",     count: orders.filter((o) => o.status === "Ready").length,    color: "text-purple-400", bg: "bg-purple-400/10", border: "border-purple-400/20" },
//             { label: "Completed", count: completedOrders,                                      color: "text-green-400",  bg: "bg-green-400/10",  border: "border-green-400/20"  },
//           ].map((s) => (
...
//               <div className="text-4xl mb-3">📋</div>
//               <div className="font-semibold text-sm">No orders yet</div>
//               <div className="text-xs mt-1">Share your QR codes so customers can order</div>
//             </div>
//           ) : (
//             <div className="space-y-2">
//               {recentOrders.map((order) => (
...
//                   <div className="flex-1 min-w-0">
//                     <div className="text-white font-semibold text-sm">
//                       {order.tableId?.name || "Unknown Table"} · {order.items.length} item{order.items.length !== 1 ? "s" : ""}
//                     </div>
//                     <div className="text-slate-400 text-xs mt-0.5 truncate">
//                       {order.items.map((i) => i.name).join(", ")}
//                     </div>
...
//                       ₹{order.totalAmount}
//                     </div>
//                     <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${STATUS_STYLE[order.status] || ""}`}>
//                       {order.status}
//                     </span>
//                   </div>
//                 </div>
...
//               { to: "/restaurant/tables",     icon: "🪑", label: "Manage Tables",   sub: `${tables.length} tables` },
//               { to: "/restaurant/orders",     icon: "📋", label: "View Orders",     sub: `${pendingOrders} pending` },
//               { to: "/restaurant/reviews",    icon: "⭐", label: "View Reviews",    sub: `${reviews.length} reviews` },
//             ].map((link) => (
//               <Link
//                 key={link.to}
//                 to={link.to}
