...
// import { useDispatch, useSelector } from "react-redux";
// import {
//   setTable, selectCartCount, selectCartTotal,
//   addToCart, updateQty, selectItemCartQty, selectItemCartId,
// } from "../../features/cart/cartSlice";
// import {
//   selectCurrentCustomer, logoutCustomer, loadCustomer,
// } from "../../features/customerAuth/customerAuthSlice";
// import { getMenuApi, createSessionApi } from "../../api/publicApi";
// import ItemDetailModal from "../../components/menu/ItemDetailModal";
// import CartDrawer from "../../components/cart/CartDrawer";
...
//   const handlePlus = (e) => {
//     // just increase quantity — do NOT open modal
//     e.stopPropagation();
//     dispatch(addToCart({ ...item, qty: 1 }));
//   };
...
//         {item.description && (
//           <p className="text-gray-500 text-xs mt-1.5 leading-relaxed line-clamp-2">{item.description}</p>
//         )}

//         {!item.isAvailable && (
//           <span className="text-red-400 text-xs font-semibold mt-1.5 block">Currently unavailable</span>
//         )}
//       </div>
...
//                   −
//                 </button>
//                 <span className="w-7 text-center text-white font-black text-sm">{cartQty}</span>
//                 <button
//                   onClick={handlePlus}
//                   className="w-9 h-9 text-white font-black text-xl flex items-center justify-center active:bg-[#e07018]"
//                 >
...
//   useEffect(() => { dispatch(loadCustomer()); }, [dispatch]);
...
//         setTableData(data.table);
//         setRestaurantId(data.restaurant._id);
//         dispatch(setTable({ tableId: data.table._id, tableName: data.table.name }));
...
//     setOrderCount((prev) => {
//     const newCount = prev + 1;
//     // save to sessionStorage inside functional update for accuracy
//     sessionStorage.setItem(`orderCount_${tableId}`, String(newCount));
//     return newCount;
//     });
//     showToast("Order placed! Kitchen is on it 🍳");
...
// const handleGoToLogin = () => {
//       console.log("Saving return URL:", location.pathname); // should print /menu/xxx/xxx
...
//   // ── Category filtering ────────────────────────────────────────
//   // When "all" selected — show all items grouped by category
//   // When specific category selected — show only that category's items
//   const getFilteredItems = () => {
//     if (searchQuery.trim()) {
//       return menuItems.filter((m) =>
//         m.name.toLowerCase().includes(searchQuery.toLowerCase())
//       );
//     }
//     if (activeCat === "all") return null; // null means show grouped view
//     return menuItems.filter((m) => {
//       const catId = (m.categoryId?._id || m.categoryId)?.toString();
//       return catId === activeCat;
//     });
...
//   const filteredItems = getFilteredItems();
//   const selectedCategory = categories.find((c) => c._id?.toString() === activeCat);
...
//       <div className="text-center">
//         <div className="text-5xl mb-4">❌</div>
//         <div className="text-white font-bold text-xl mb-2">Table Not Found</div>
//         <div className="text-gray-400 text-sm">{error}</div>
//       </div>
//     </div>
//   );
...
//       {/* Toast */}
//       {toast && (
//         <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-999 px-5 py-2.5 rounded-xl font-semibold text-sm text-white shadow-2xl whitespace-nowrap
//           ${toast.type === "success" ? "bg-[#25C866]" : "bg-red-500"}`}>
//           {toast.msg}
//         </div>
//       )}
...
//               </span>
//             </div>
//             <h1 className="text-white font-bold text-xl leading-tight truncate">{restaurant?.name}</h1>
//             {restaurant?.description && (
//               <p className="text-gray-500 text-xs mt-0.5 truncate">{restaurant.description}</p>
//             )}
//           </div>
//           <div className="flex items-center gap-2 shrink-0">
//             <button
...
//               />
//               {searchQuery && (
//                 <button onClick={() => setSearchQuery("")} className="text-gray-500">✕</button>
//               )}
//             </div>
//           </div>
//         )}
...
//             <>
//               <span className="text-gray-500 text-xs">
//                 👋 <span className="text-gray-300 font-medium">{currentCustomer.username}</span>
//               </span>
//               <button
//                 onClick={async () => { await dispatch(logoutCustomer()); }}
//                 className="text-red-400 text-xs font-medium"
...
//           ) : (
//             <>
//               <span className="text-gray-600 text-xs">Login to rate dishes ⭐</span>
//               <div className="flex gap-4">
//                 {/* <button
//                   onClick={() => navigate("/customer/login", { state: { from: location.pathname, restaurantId } })}
//                   className="text-[#FC8019] text-xs font-semibold"
//                 >
//                   Login
//                 </button>
//                 <button
//                   onClick={() => navigate("/customer/register", { state: { from: location.pathname, restaurantId } })}
//                   className="text-[#FC8019] text-xs font-semibold"
//                 >
//                   Register
//                 </button> */}
//                 {/* // 4. Use these handlers in the customer bar buttons */}
// <button onClick={handleGoToLogin} className="text-[#FC8019] text-xs font-semibold">Login</button>
// <button onClick={handleGoToRegister} className="text-[#FC8019] text-xs font-semibold">Register</button>
//               </div>
//             </>
//           )}
//         </div>
...
//                   key={cat._id}
//                   onClick={() => setActiveCat(cat._id?.toString() || "all")}
//                   className={`shrink-0 px-4 py-3 text-xs font-semibold whitespace-nowrap border-b-2 transition-all
//                     ${activeCat === (cat._id?.toString() || "all")
//                       ? "text-[#FC8019] border-[#FC8019]"
//                       : "text-gray-500 border-transparent"
//                     }`}
...
//             <div className="px-4 pt-4 pb-2">
//               <span className="text-gray-500 text-xs">
//                 {filteredItems?.length || 0} result{filteredItems?.length !== 1 ? "s" : ""}
//               </span>
//             </div>
//             {filteredItems?.length === 0 ? (
//               <div className="text-center py-16 text-gray-600">
//                 <div className="text-4xl mb-3">🔍</div>
//                 <div className="font-semibold text-gray-400">No dishes found</div>
//                 <div className="text-sm mt-1">Try a different search term</div>
//               </div>
//             ) : (
//               <div className="mt-2">
//                 {filteredItems?.map((item) => (
//                   <ItemRow key={item._id} item={item} onOpen={setSelectedItem} />
//                 ))}
//               </div>
//             )}
//           </>
...
//       {/* {showAuth && (
//         <CustomerAuthModal
//         //   one new line down restaurantId
//           restaurantId={restaurantId}
//           onClose={() => { setShowAuth(false); setPendingReviewItem(null); }}
//         />
//       )} */}
//       {showAuth && (
//   <CustomerAuthModal
